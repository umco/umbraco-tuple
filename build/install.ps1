...
$propertyListFolder = $appPluginsFolder.ProjectItems | Where-Object { $_.Name -eq "Tuple" }

if (!$propertyListFolder)
{
	$newPackageFiles = "$installPath\Content\App_Plugins\Tuple"
	...
	if ($projectPathExists) {
		Write-Host "Updating Tuple App_Plugin files using PS as they have been excluded from the project"
